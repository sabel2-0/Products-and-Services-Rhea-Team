    ...
    public class SellerController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            try
            {
                ...
                // 1. Fetch all ratings/reviews and calculate the averages (From File 1)
                ...
                // 2. Fetch Variant Snapshots for Dynamic Minimum Price and Fallback Image (From File 2)
                ...
                // Map everything to the products
                foreach (var p in products)
                {
                    // Map Dynamic Price & Image
                    ...
                    {
                        // If no reviews exist, default to 0 count and 5.0 stars
                        p.ReviewCount = 0;
                        p.AverageRating = 5.0;
                    }
                }
            ...
            string[]? variantWidths)
        {
            try
            {
                ...
                // Fallback when browser sends multiple files
                ...
                // Build per-color variants for normalization
                ...
                {
                    // ENFORCED RULE: All new products must go to pending for admin approval
                ...
                // Post-Creation Redirect to Size Guide
        ...
        private static SizeGuidePayload ParseSizeGuidePayload(string? rawJson, string? fallbackTitle)
        {
            ...
            try
            {
                ...
                if (doc.RootElement.ValueKind != JsonValueKind.Array) return payload;
        ...
        public async Task<IActionResult> ViewRatings(int id)
        {
            ...
            // Dynamic minimum price for the View Ratings Page
        ...
        private static bool IsTransientDatabaseException(Exception ex)
        {
            ...
            return ex.InnerException is InvalidOperationException innerIoe && innerIoe.Message.Contains("connection from the pool", StringComparison.OrdinalIgnoreCase);
        }
