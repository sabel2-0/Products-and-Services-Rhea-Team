    ...
    public class SellerController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            try
            {
                ...
                // NEW: Fetch all ratings/reviews and calculate the averages
                // Assumes your Review model has a property called 'Rating'
                ...
                foreach (var p in products)
                {
                    ...
                    // NEW: Map Ratings to the product model for the UI
            ...
            string[]? variantWidths)
        {
            try
            {
                ...
                // NEW: Added Renew condition to prevent database crash
                else if (mode == "renew" && model.ProductId > 0)
                {
                    ...
                    if (existing != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(model.ImagePath))
                            existing.ImagePath = model.ImagePath;
                        await _db.SaveChangesAsync();
                        _logger.LogInformation($"Renewed product {model.ProductId}");
                ...
                else
                {
                    ...
                    await _db.SaveChangesAsync();
                    productId = model.ProductId;
                    _logger.LogInformation($"Created new product {productId} with pending status");
                ...
                if (mode == "edit")
                    ...
                    TempData["SuccessMessage"] = "Product renewed successfully.";
                else
                    TempData["SuccessMessage"] = "Product added successfully and is pending approval.";
